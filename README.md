...
- BehaviorParameters
Компонент для настройки поведения и свойств мозга экземпляра агента.
...
- В сцену был добавлен белый куб, который стал второй целью шара.
 ![14](https://user-images.githubusercontent.com/106344305/197402195-d0da25cf-e4ab-4f8c-a18d-98f558bf4f4b.PNG)
- В скрипте была добавлена новая цель (белый куб). Для новой цели был написан код, который позволил получить необходимый результат - шар начал видеть эту новую цель и катиться за ней.
```C#
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
...
public class RollerAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        if(distanceToTarget < 1.42f || distanceToTarget < 1.42f)
        {
            SetReward(1.0f);
            EndEpisode();
        }
...
```
- После этого был запущен процесс переобучения модели шара.

- После выполненных действий удалось создать новую модель поведения шара, который в результате перемещался между двумя кубами разного цвета.



## Выводы

Игровой баланс — субъективное «равновесие» между персонажами, командами, тактиками игры и другими игровыми объектами. Игровой баланс — одно из требований к «честности» правил. Существуют следующие парадигмы баланса: метод от образа, метод от характеристик. Выделяют виды баланаса: Сила, Время, Пространство, Сложность. Системы машинного обучения могут быть использованы для того, чтобы выявить в играх дисбаланс.
В данной работе я научилась создавать системы машинного обучения и интегрировать их в Unity.
